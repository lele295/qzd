 ...
 */
    ...
    this.selectorPIdStr = 'ebuycity-level-one'+ this.config.suffix;
    this.selectorCIdstr = 'ebuycity-level-two' + this.config.suffix;
    this.init();
}
...
$(function(){
    ...
    $(".branch").click(function(){
        ...
        var bank = $('#bank').val();
        var bankName = $('#bank option:selected').text().trim();
        var bankLength = bankName.length;

        //ajax请求数据库查询支行
        if(city != '' && bank != ''){
            $.post("/wx/loan/bank-branch",{"city":city,"bank":bank},function(data){
                var src = data.branch_info;
                //console.log(src);
                var options = '';
                //如果有支行
                for(var x in src){
                    if(src[x].BANKNAME.indexOf('有限公司') == -1) {
                        if(src[x].BANKNO == '308584000013') {
                            options += '<option value="308584000013">' + '招商银行' + '</option>';
                        }else if(src[x].BANKNO == '307584007998'){
                            options += '<option value="308584000013">' + '平安银行' + '</option>';
                        }else if(src[x].BANKNO == '307584099984'){
                            options += '<option value="308584000013">' + '平安银行股份有限公司' + '</option>';
                        }else{
                            options += '<option value="' + src[x].BANKNO + '">' + src[x].BANKNAME.substr(bankLength-6, 20) + '</option>';
                        }
                    }else{
                        options += '<option value="'+src[x].BANKNO+'">'+src[x].BANKNAME.substr(bankLength, 20)+'</option>';
                    }
                }
                $("#bankBranch").empty().append(options);
        ...
        if(city != '' && bank != ''){
            $.post("/wx/loan/bank-branch",{"city":city,"bank":bank},function(data){
                ...
                if(data.error == 1){
                    ...
                    //如果有支行
                    for(var x in src){
                        if(src[x].BANKNAME.indexOf('云南省农村信用社')>= 0){
                            options += '<option value="'+src[x].BANKNO+'">'+src[x].BANKNAME.substr(0,20)+'</option>';
                        }
                        else if(src[x].BANKNAME.indexOf('有限公司') == -1){
                            options += '<option value="'+src[x].BANKNO+'">'+src[x].BANKNAME.substr(bankLength-6,20)+'</option>';
                        }else{
                            options += '<option value="'+src[x].BANKNO+'">'+src[x].BANKNAME.substr(bankLength,20)+'</option>';
                        }
    ...
    $("#ebuycity-level-one").change(function(){
        var city = $('#bankCity').val();
        var bank = $('#bank').val();
        var bankName = $('#bank option:selected').text().trim();
        var bankLength = bankName.length;
    ...
    $("#ebuycity-level-two").change(function(){
        var city = $('#bankCity').val();
        var bank = $('#bank').val();
        var bankName = $('#bank option:selected').text().trim();
        var bankLength = bankName.length;
