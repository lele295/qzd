function common_sure(ethis){
    ethis.oldvalue = ethis.value.concat([]);
    ethis.oldtext = ethis.text.concat([]);
    var sure_val = "";
    for(var i=0; i<ethis.level; i++){
        if(i!=0){
            sure_val+=","
        }
        sure_val += ethis.value[i];
    }
    if(ethis.oldtext==""){
        ethis.oldtext = "请选择邻近城市支行";
    }
    ethis.trigger.text(ethis.oldtext);
    ethis.trigger.next('input').val(sure_val);
}
	...
	MobileSelectArea.prototype = {
		...
		bindEvent: function() {
			...
			this.trigger[_this.settings.eventName](function(e) {
				...
				for (var i = 0; i < _this.level; i++) {
                    if(_this.level == 1){
                        dlgContent += '<div style="width:100%"></div>'
                    }else{
					    dlgContent += '<div></div>'
                    };
				};
		...
		submit: function() {
            ...
            for(var i=0; i<this.level; i++){
                ...
                sure_val += this.value[i];
            }
            this.trigger.val(this.text.join(this.separator));
            this.trigger.attr('data-value', sure_val);
            */
            /*
